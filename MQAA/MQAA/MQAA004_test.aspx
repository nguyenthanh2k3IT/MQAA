    ...
    <script>
        ...
        function sortAndPopulateTable() {
            ...
            $.each(tableData, function (index, row) {
                ...
                var statusCell = $('<td>').text(row.STATUS || '').css('cursor', 'pointer').on('click', function() {
                    redirectToMQAA(row);
                });
        ...
        function redirectToMQAA(row) {
            // Get current search criteria
            var factory = $('#cboFCT_LIST').val();
            var plant = $('#cboPLANT_LIST').val();
            var line = $('#cboLINE_LIST').val();
            var group = $('#cboGROUP_LIST').val();
            var section = $('#cboSECTION_LIST').val();
            var monthly = $('#cbpMonthly').val();
            var weekly = $('#cboWeekly').val();

            // Construct URL with query parameters
            var url = '/MQAA/MQAA001.aspx?' +
